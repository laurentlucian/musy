...
import { likedTracks } from "~/lib.server/db/schema";
import { SpotifyApiError } from "~/lib.server/sdk/spotify/errors";
import { db } from "~/lib.server/services/db";
import { getPlaylistTracksFromDB } from "~/lib.server/services/db/playlists";
...
export async function likePlaylistTracks({
  ...
  try {
    ...
    log(
      `completed like playlist tracks: ${likedCount} liked`,
      "playlist-actions",
    );
...
export async function unlikePlaylistTracks({
  ...
  try {
    log(
      `starting unlike playlist tracks for ${playlistId}`,
      "playlist-actions",
    );
    ...
    log(
      `completed unlike playlist tracks: ${unlikedCount} unliked`,
      "playlist-actions",
    );
