...
export class Track extends ReadWriteBaseClient {
  ...
  checkUsersSavedTracks(ids: string[]) {
    return this.get<Array<boolean>>(`/me/tracks/contains`, {
      ids: joinIdsArrayToString(ids),
    });
  }
  ...
  getMultipleTracksAudioFeatures(ids: string[]) {
    return this.get<AudioFeaturesArray>(`/audio-features`, {
      ids: joinIdsArrayToString(ids),
    });
  }
   ...
   */
  getRecommendations(
    seedParams: RecommendationSeedParams,
    optionalParams: RecommendationOptionalParams,
  ) {
    return this.get<Recommendations>(`/recommendations`, {
      ...seedParams,
      ...optionalParams,
    });
  }
