...
export class User extends ReadWriteBaseClient {
  ...
  getUserTopItems(
    ...
    optionalParams?: TopItemsOptionalParams,
  ) {
   ...
   */
  getFollowedArtists(
    type: FollowedArtistType,
    optionalParams?: FollowedArtistOptionalParams,
  ) {
    return this.get<FollowedArtist>(`/me/following`, {
      type,
      ...optionalParams,
    });
  }
  ...
  followArtistsOrUsers(type: ArtistsUsersType, ids: string[]) {
    return this.put(`/me/following${generateQueryParametersString({ type })}`, {
      ids,
    });
  }
  ...
  unfollowArtistsUsers(type: ArtistsUsersType, ids: string[]) {
    return this.delete(
      `/me/following${generateQueryParametersString({ type })}`,
      { ids },
    );
  }
  ...
  checkIfUserFollowsArtistsOrUsers(type: ArtistsUsersType, ids: string[]) {
    return this.get<Array<boolean>>("/me/following/contains", {
      type,
