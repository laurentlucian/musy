...
export class SpotifyStrategy extends OAuth2Strategy<Session, SpotifyProfile, SpotifyExtraParams> {
  name = 'spotify';
  ...
  protected async handleResult(
    ...
    if (options.failureRedirect && hasErrored)
      return this.failure(result as string, req, this.sessionStorage, options);

    if (hasErrored) return null;

    if (options.successRedirect && !hasErrored)
      return this.success(result as Session, req, this.sessionStorage, options);
  ...
  async getSession(req: Request, checkOptions: CheckOptions = {}): Promise<Session | null> {
