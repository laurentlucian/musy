...
import { NetworkError, SpotifyApiError } from "../errors/index.js";
import type { BearerToken, SpotifiedResponse } from "../types/index.js";
...
export default class RequestMaker {
  ...
  private prepareHeadersAndBody(requestParams: {
    ...
    if (
      ...
      isFormUrlEncoded(customHeaders["Content-Type"])
    ) {
  ...
  async makeRequest<T = any>(
    ...
    try {
      const url = requestParams.url.startsWith("http")
        ? requestParams.url
        : `${SPOTIFY_API_URL}${requestParams.url}`;
