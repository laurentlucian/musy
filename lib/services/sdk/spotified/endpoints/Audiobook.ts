...
export class Audiobook extends ReadWriteBaseClient {
  ...
  getSeveralAudiobooks(
    ids: string[],
    optionalParams?: GetAudiobookOptionalParams,
  ) {
    return this.get<Audiobooks>(`/audiobooks`, {
  ...
  getAudiobookChapters(
    id: string,
    optionalParams?: GetAudiobookChaptersOptionalParams,
  ) {
    return this.get<AudiobookChapters>(
  ...
  saveAudiobooksforCurrentUser(ids: string[]) {
    return this.put(
      `/me/audiobooks${generateQueryParametersString({ ids: joinIdsArrayToString(ids) })}`,
    );
  }
