...
export class Show extends ReadWriteBaseClient {
  ...
  saveShowsForCurrentUser(ids: string[]) {
    return this.put(
      `/me/shows${generateQueryParametersString({ ids: joinIdsArrayToString(ids) })}`,
    );
  }
  ...
  removeUserSavedShows(
    ids: string[],
    optionalParams: RemoveUsersShowsOptionalParams,
  ) {
    return this.delete(
  ...
  checkUserSavedShows(ids: string[]) {
    return this.get<Array<boolean>>(`/me/shows/contains`, {
      ids: joinIdsArrayToString(ids),
    });
  }
}
